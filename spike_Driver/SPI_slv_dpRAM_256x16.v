/* ----------------------------------------------------------------------------
MODULO de interface serial             == SPI_slv_dpRAM ==
autor:   João Ranhel
versão:  1.01                          data: 03_03_2019
--
...
// pars *passados: larg WORD mem, larg Add_bus mem, faixa ends dessa interface
  parameter SZWDA = 8;                 // largura default 8 bits
  parameter SZADDA = 7;                // largura bus ends RAM (Add_a)
  parameter SZWDB = 8;                 // largura default 8 bits  
  parameter SZADDB = 7;                // largura bus ends RAM (Add_b)-SPI
  parameter ADD0 = 14'h0100;           // end inicial desse bloco de RAM
// parametros locais
  localparam CMDLEN = 16;              // tam reg de comando: SRcmd
  localparam CMDcntLEN = 5;            // num bits do contador de comando
  localparam BITcntLEN = 6;            // num bits do contador de BITs
...
// p/ zerar SRdata E SRcmd - independente do comprimento em bits deles
  localparam [SZWDB-1:0] RESSR = {{(SZWDB-4){1'b0}}, 4'd0};
  localparam [CMDLEN-1:0] RESCMD = {{(CMDLEN-4){1'b0}}, 4'd0};
  localparam [CMDcntLEN-1:0] REScntCMD = {{(CMDcntLEN-4){1'b0}}, 4'b0000};
  localparam [CMDcntLEN-1:0] LOCKcntCMD = {{(CMDcntLEN-4){1'b1}}, 4'b1111};
  localparam [BITcntLEN-1:0] REScntBIT = {{(BITcntLEN-4){1'b0}}, 4'b0000};
  localparam [BITcntLEN-1:0] LOCKcntBIT = {{(BITcntLEN-4){1'b1}}, 4'b1111};
  localparam [SZADDB-1:0] RESADD = {{(SZADDB-4){1'b0}}, 4'd0};
// esses registradores tem a ver com a interface serial de config
  reg [SZWDB-1:0] SRin, SRout;         // shiftReg entrada e saída
  reg [CMDLEN-1:0] SRcmd;              // ShiftReg in comando
  reg [CMDLEN-3:0] Radd_b;             // reg end mem port-b (via SPI)
  reg [CMDcntLEN-1:0] CntCMD;          // conta bits de COMANDO
  reg [BITcntLEN-1:0] CntBIT;          // conta bits de DADOS 
  reg nxtWD, fWRDok;                   // flag p/ indicar new WD na ↓sck
  wire mCS;                            // mem ChipSelect dessa DPRAM

// lógica combinacional deste módulo
  assign miso = mCS ? SRout[SZWDB-1]: 1'b0;
  assign mCS = ((Radd_b>=ADD0) && (Radd_b<=ADDF))? 'b1: 'b0;
...
always @(negedge sck or posedge nss)
  begin
  if(nss == 'b1)                       // ... em reset
    Radd_b <= RESADD;                  // inicializa Radd_b com zero  
  else if (CntCMD == (CMDLEN-2))       // se vieram CMDLEN-2 bits cmd
    Radd_b <= SRcmd[CMDLEN-3:0];       // carrega Radd_b c/ bits do SRcmd    
  else if (CntBIT == 2)                // se estiver no 2o. bit do CntBIT
    if ((Radd_b<ADDF)&&((SRcmd[1]=='b1)||(fWRDok=='b1)) ) // LER ou fWRDok=1 
      Radd_b <= Radd_b + 'b1;          // inc endereço memoria Radd_b
  end
